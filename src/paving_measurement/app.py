...
def get_satellite_image(client: MapboxClient, latitude: float, longitude: float, zoom: int) -> Image.Image:
    ...
    for x_offset in range(-1, 2):
        for y_offset in range(-1, 2):
            mosaic.paste(client.download_tile(center_x + x_offset, center_y + y_offset, zoom), ((x_offset + 1) * tile_size, (y_offset + 1) * tile_size))
    return mosaic
